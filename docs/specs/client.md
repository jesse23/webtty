...
| Tighter fit and padding colour | Grid sized from the full container without `FitAddon`'s 15px scrollbar reserve, shrinking the strip at the edges to under one cell; optional `theme.padding` colours what is left (default: `theme.background`) | [ADR 033](../adrs/033.client.padding-fit.md) | ✅ |
| Configurable padding | Top-level `padding` (px, default `0`) reserves space around the canvas on all sides; the grid is fitted to what remains; the top and left gap is exactly `padding` and the leftover goes to the right and bottom | [ADR 034](../adrs/034.client.padding-config.md) | ✅ |
