...
**Config interaction:** `config.cursorStyle` and `config.cursorBlink` set the initial values at Terminal construction. DECSCUSR overrides them at runtime. The two compose cleanly: config is the default, apps switch dynamically as needed.
...
- [ADR 008 — Config](008.webtty.config.md): established `cursorBlink` as a config key; `cursorStyle` is added alongside it
