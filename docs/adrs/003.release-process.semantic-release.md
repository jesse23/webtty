...
wtty targets developers who already have Node.js or Bun installed. Distribution via `npx wtty` is the natural fit — no binary downloads, no install step beyond what developers already have. A release means a GitHub Release with a git tag and release notes. The version should be derived automatically from what changed, not set manually.

The cfx repo solved the same versioning problem with semantic-release (ADR 004): conventional commit prefixes in PR/MR titles drive version bumps post-merge, git tags are the sole version record, no release commits. That pattern maps directly to wtty.
...
- **Pros**: Zero developer overhead beyond PR title. GitHub Release + release notes fully automated. Battle-tested. No version conflicts between concurrent PRs. No release commits — clean one-commit-per-PR history on `main`. `GITHUB_TOKEN` is provided automatically — no secrets to configure.
...
**Good**: Zero developer overhead — write a conventional PR title, everything else is automated. GitHub Release auto-created with release notes. Clean `main` history. `GITHUB_TOKEN` is automatic. Follows semantic-release's own recommended approach (tag-only, no commit-back).

**Bad**: `package.json` version stays at `0.0.0-development`. No `CHANGELOG.md` in repo. Requires conventional commit discipline in PR titles.
...
All three plugins are bundled with `semantic-release` — no additional packages needed. `@semantic-release/git` and `@semantic-release/changelog` intentionally omitted (tag-only).
