...
`@semantic-release/npm` with `npmPublish: false` writes the bumped version to `package.json` in CI but does not publish. The actual publish is done by `npm publish` in a separate workflow step. A `prepublishOnly` script in `package.json` strips `scripts` and `devDependencies` in-place before npm reads the package. `@semantic-release/git` and `@semantic-release/changelog` intentionally omitted (tag-only).

`prepublishOnly` in `package.json` (runs automatically before `npm publish`):

```json
"prepublishOnly": "node -e \"const fs=require('fs'),p=JSON.parse(fs.readFileSync('package.json','utf8'));delete p.scripts;delete p.devDependencies;fs.writeFileSync('package.json',JSON.stringify(p,null,2)+'\\n');\""
```
...
- semantic-release writes the bumped version to `package.json`, then `npm publish` triggers `prepublishOnly` which strips dev fields before publishing
