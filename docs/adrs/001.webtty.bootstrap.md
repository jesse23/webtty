...
**Why single-port?** Simplifies reverse proxy setup (nginx, ngrok, Cloudflare Tunnel) — one upstream, no separate ports for assets vs API vs WebSocket. Same pattern used by ttyd and the ghostty-web demo.

**Why `ws` over Socket.IO?** Minimal footprint, no client-side library requirement, sufficient for raw PTY streaming. Socket.IO's rooms, namespaces, and fallback transports are unnecessary overhead for this use case.
