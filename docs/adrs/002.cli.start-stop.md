...
- **Pros**: Fully cross-platform — no signals, no PID file. Server owns its cleanup. Connection refused = not running, no separate state to track. Consistent with the existing HTTP surface of the server. The same interface is trivially debuggable with `curl`. Using HTTP instead of a Unix socket avoids platform-specific socket path handling and keeps the CLI surface area identical on Mac, Linux, and Windows.
